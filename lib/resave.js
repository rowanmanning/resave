 ...
 *     The full path to the file to create a bundle for.
...
async function middleware(createBundle, options, request, response, next) {
	try {
		...
		const bundlePath = options.bundles[request.path];
		if (!bundlePath) {
			return next();
		}
		...
		let bundleContent;
		try {
			const fullBundlePath = path.join(options.basePath, bundlePath);
			bundleContent = await createBundle(fullBundlePath, options);
		...
		if (options.savePath) {
			try {
				const fullSavePath = path.join(options.savePath, request.path);
